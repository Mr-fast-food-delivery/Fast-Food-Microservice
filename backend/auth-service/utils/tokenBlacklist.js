...
const blacklistToken = async (token, expireInSeconds) => {
  try {
    ...
    if (isRedisAvailable()) {
      const ttl =
        expireInSeconds || process.env.TOKEN_BLACKLIST_EXPIRE || 86400; // Default: 24 hours
      ...
      if (tokenExpiry) {
        const secondsToExpiry = Math.max(
          1,
          Math.floor((tokenExpiry - new Date()) / 1000)
        );
    ...
    // Fallback to memory blacklist
    const expiry =
      expireInSeconds || process.env.TOKEN_BLACKLIST_EXPIRE || 86400;
