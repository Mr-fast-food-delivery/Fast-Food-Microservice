  ...
  try {
    ...
    for (const serviceName of serviceNames) {
      ...
      if (!existingAccount) {
        ...
        const clientId = `svc_${require("crypto")
          .randomBytes(16)
          .toString("hex")}`;
        ...
        console.log(
          `Created service account for ${serviceName} with clientId: ${clientId}`
        );
        console.log(`Initial client secret: ${clientSecret}`);
        console.log(
          "IMPORTANT: Store this client secret securely, it won't be shown again"
        );
      } else {
