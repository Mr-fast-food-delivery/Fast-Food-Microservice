...
const ORDER_SERVICE_URL =
  process.env.REACT_APP_ORDER_SERVICE_URL || "http://localhost:3003";
const RESTAURANT_SERVICE_URL =
  process.env.REACT_APP_RESTAURANT_SERVICE_URL || "http://localhost:3002";
...
module.exports = function (app) {
  ...
  app.use(
    ...
    createProxyMiddleware({
      ...
      onProxyReq: (proxyReq, req, res) => {
        console.log(
          `Auth Service Proxy Request: ${req.method} ${req.path} -> ${getTarget(
            AUTH_SERVICE_URL
          )}`
        );
      },
      onProxyRes: (proxyRes, req, res) => {
        console.log(
          `Auth Service Proxy Response: ${proxyRes.statusCode} for ${req.method} ${req.path}`
        );
      },
      pathRewrite: {
        "^/api/auth": "/api/auth",
  ...
  app.use(
    ...
    createProxyMiddleware({
      ...
      onProxyReq: (proxyReq, req, res) => {
        console.log(
          `Order Service Proxy Request: ${req.method} ${
            req.path
          } -> ${getTarget(ORDER_SERVICE_URL)}`
        );
      },
      pathRewrite: {
  ...
  app.use(
    ...
    createProxyMiddleware({
      ...
      onProxyReq: (proxyReq, req, res) => {
        console.log(
          `Restaurant Service Proxy Request: ${req.method} ${
            req.path
          } -> ${getTarget(RESTAURANT_SERVICE_URL)}`
        );
      },
      pathRewrite: {
