...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        try {
          ...
          if (storedProfileStr) {
            ...
            console.log(
              "Found stored profile data on login:",
              storedProfileData
            );
          }
        } catch (profileError) {
            ...
            if (response.success) {
              ...
              if (storedProfileData) {
                ...
                userData = {
                  ...
                  profilePicture:
                    storedProfileData.profilePicture ||
                    storedProfileData.profilePic ||
                  ...
                  defaultDeliveryAddress:
                    storedProfileData.defaultDeliveryAddress ||
                    userData.defaultDeliveryAddress,
                };

                console.log(
                  "Merged user data with priority to backend for auth data:",
                  userData
                );
                ...
                try {
                  ...
                  console.warn(
                    "Failed to synchronize profile to server:",
                    updateError
                  );
                }
              }
              ...
              setTimeout(() => {
                if (userData && userData.userType) {
                  ...
                  console.log(
                    "Double-checked and enforced user type:",
                    backendUserType
                  );
                }
              }, 100);
    ...
    try {
      ...
      if (email.endsWith("@example.com")) {
        const userType = email.startsWith("admin")
          ? "admin"
          : email.startsWith("restaurant")
          ? "restaurant-admin"
          : email.startsWith("delivery")
          ? "delivery-personnel"
          : "customer";
      ...
      if (response.success) {
        ...
        console.log(
          "Processing successful response data:",
        ...
        if (response.data && response.data.token && response.data.user) {
          ...
          console.log(
            "Examining response.data structure:",
        ...
        if (!token) {
          console.error(
            "Could not extract token and user from response:",
            JSON.stringify(response, null, 2)
          );
          return {
            success: false,
          ...
          if (tokenParts.length === 3) {
            ...
            if (
              ...
              (!tokenPayload.permissions ||
                !tokenPayload.permissions.includes("user:create"))
            ) {
        ...
        try {
          ...
          if (currentProfile) {
            ...
            console.log(
              "Updated userProfile in localStorage with correct userType:",
              user.userType
            );
          } else {
  ...
  const getUsersDirectly = async (
    ...
    try {
      console.log(
        `Getting users with params: page=${page}, limit=${limit}, search=${search}, userType=${userType}`
      );
      ...
      try {
        ...
        if (storedUsers) {
          ...
          console.log(
            `Found ${createdUsers.length} created users in localStorage`
          );
        }
      } catch (e) {
      ...
      try {
        ...
        if (storedRegistered) {
          ...
          console.log(
            `Found ${registeredUsers.length} registered users in localStorage`
          );
        }
      } catch (e) {
      ...
      let allUsers = [...mockUsers, ...createdUsers, ...registeredUsers];
      ...
      if (userType && userType.trim() !== "") {
        ...
        console.log(
          `Filtered by userType '${normalizedUserType}', remaining: ${allUsers.length} users`
        );
      }
      ...
      if (search && search.trim() !== "") {
        ...
        console.log(
          `Filtered by search '${search}', remaining: ${allUsers.length} users`
        );
      }
      ...
      console.log(
        `Paginated users: ${paginatedUsers.length} (page ${page}, limit ${limit})`
      );
  ...
  const hasRole = (role) => {
    ...
    const normalizedUserType = (currentUser.userType || "")
      .trim()
      .toLowerCase();
  ...
  const hasAnyRole = (roles) => {
    ...
    return roles.some(
      (role) => (role || "").trim().toLowerCase() === normalizedUserType
    );
  };
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      if (response.success) {
        ...
        throw new Error(
          response.message || "Failed to update profile on server"
        );
      }
    } catch (error) {
  ...
  const forceUpdateUserType = (user, forceType = null) => {
    ...
    try {
      ...
      if (storedProfileStr) {
        ...
        console.log(
          "forceUpdateUserType - Updated localStorage userType to:",
          userType
        );
      }
    } catch (e) {
